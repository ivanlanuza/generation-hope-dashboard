...
import {
  buildYearlyClassrooms,
  buildYearlyStudents,
  mapBuildsBySchool,
  mapClassroomsBySchool,
  mapImpactsBySchool,
  buildTreemapByIslandRegion,
  buildTreemapByDonor,
} from "@/lib/aggregations";
import {
  deriveBuildsRows,
  deriveDonationsByDonor,
  deriveDonorRows,
  deriveSchoolsRows,
} from "@/lib/selectors";
...
export default function useDashboardData() {
  ...
  const donationsByDonor = useMemo(
    () => deriveDonationsByDonor(classrooms),
    [classrooms]
  );
  const donorRows = useMemo(
    () => deriveDonorRows(donors, donationsByDonor),
    [donors, donationsByDonor]
  );
  const schoolsRows = useMemo(
    () => deriveSchoolsRows(schools, builds),
    [schools, builds]
  );
  ...
  const classroomsBySchoolMap = useMemo(
    () => mapClassroomsBySchool(classrooms),
    [classrooms]
  );
  const impactsBySchoolMap = useMemo(
    () => mapImpactsBySchool(impacts),
    [impacts]
  );

  const yearlyImpactData = useMemo(
    () => buildYearlyClassrooms(builds),
    [builds]
  );
  const yearlyStudentsData = useMemo(
    () => buildYearlyStudents(impacts),
    [impacts]
  );
  ...
  const totals = useMemo(
    () => ({
      ...
      studentsImpacted: impacts.reduce(
        (sum, r) => sum + Number(r.impactedStudentCount || 0),
        0
      ),
      totalConstructionCost: builds.reduce(
  ...
  const buildsTreemapData = useMemo(
    () => buildTreemapByIslandRegion(schools, builds),
    [schools, builds]
  );

  const donationTreemapData = useMemo(
    () => buildTreemapByDonor(classrooms),
    [classrooms]
  );
